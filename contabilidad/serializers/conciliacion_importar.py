...
class ConConciliacionImportarSerializer(serializers.Serializer):
    """
    Define la estructura del Excel de importación de conciliaciones y la lógica
    de creación bulk.
    ...
    campos_excel = (
        ...
        ('cuenta_banco.id', 'Cuenta banco'),
    )
    campos_requeridos = {'fecha_desde', 'fecha_hasta'}
    ...
    def procesar_lote(self, filas_validas):
        ...
        mapa_cuenta_banco = self._mapa_fk(filas_validas, 'cuenta_banco.id', GenCuentaBanco)
        ...
        for idx, datos in filas_validas:
            try:
                cuenta_banco = self._fk_opcional(datos.get('cuenta_banco.id'), mapa_cuenta_banco, 'Cuenta banco')
                nuevos.append(ConConciliacion(
        ...
        if nuevos:
            ConConciliacion.objects.bulk_create(nuevos, batch_size=self.BATCH_BULK_CREATE)
        return len(nuevos), []
