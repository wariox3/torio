...
from humano.models import HumContrato, HumLiquidacion


class HumLiquidacionImportarSerializer(serializers.Serializer):
    """
    Define la estructura del Excel de importación de liquidaciones y la lógica
    de creación bulk.
    ...
    campos_requeridos = {'contrato.id', 'fecha', 'fecha_desde', 'fecha_hasta'}
    ...
    def procesar_lote(self, filas_validas):
        ...
        if nuevos:
            HumLiquidacion.objects.bulk_create(nuevos, batch_size=self.BATCH_BULK_CREATE)
        return len(nuevos), []
