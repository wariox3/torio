...
class WebhookRededocTests(TenantTestCase):
    ...
    def test_la_prueba_con_firma_valida_responde_200_sin_tocar_el_documento(self):
        with mock.patch('general.tasks.notificar_documento.delay') as delay:
            with self.captureOnCommitCallbacks(execute=True):
                respuesta = self._llamar(tipo='prueba')

        self.assertEqual(respuesta.status_code, 200)
        self.assertEqual(respuesta.data, {'detail': 'Aviso de prueba recibido.'})
        self.documento.refresh_from_db()
        self.assertFalse(self.documento.estado_electronico)
        self.assertFalse(self.documento.estado_electronico_notificado)
        delay.assert_not_called()

    def test_la_prueba_no_necesita_documento(self):
        crudo = json.dumps({'tipo': 'prueba', 'cliente': self.tenant.id}).encode()
        self.assertEqual(self._enviar(crudo).status_code, 200)

    def test_la_prueba_ignora_un_documento_que_no_existe(self):
        self.assertEqual(self._llamar(tipo='prueba', documento=str(uuid.uuid4())).status_code, 200)

    def test_la_prueba_con_un_cliente_que_no_existe_lo_dice(self):
        """Con la firma válida quien pregunta es rededoc: se le puede decir qué falla."""
        respuesta = self._llamar(tipo='prueba', cliente=999999)
        self.assertEqual(respuesta.status_code, 404)
        self.assertEqual(respuesta.data, {'detail': 'El cliente no existe.'})

    def test_la_prueba_sin_cliente_responde_400(self):
        respuesta = self._enviar(b'{"tipo": "prueba"}')
        self.assertEqual(respuesta.status_code, 400)
        self.assertIn('cliente', respuesta.data)

    def test_la_prueba_con_otro_secreto_responde_401(self):
        respuesta = self._enviar(self._cuerpo(tipo='prueba'), secreto='otro')
        self.assertEqual(respuesta.status_code, 401)
        self.assertEqual(respuesta.data, {'detail': 'Firma inválida.'})

    def test_la_prueba_vieja_responde_401(self):
        respuesta = self._enviar(self._cuerpo(tipo='prueba'), fecha=str(int(time.time()) - 301))
        self.assertEqual(respuesta.status_code, 401)

    def test_los_demas_avisos_siguen_exigiendo_documento(self):
        crudo = json.dumps({'tipo': 'notificacion', 'cliente': self.tenant.id}).encode()
        respuesta = self._enviar(crudo)
