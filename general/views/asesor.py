...
from general.serializers import GenAsesorSeleccionarSerializer
...
class GenAsesorViewSet(viewsets.GenericViewSet):
    serializer_class = GenAsesorSeleccionarSerializer

    @extend_schema(parameters=_SELECCIONAR_PARAMS, responses=GenAsesorSeleccionarSerializer(many=True))
    @action(detail=False, methods=['get'], pagination_class=SeleccionarPaginacion)
    def seleccionar(self, request):
        qs = GenAsesor.objects.all()
