"""
Formato del pago: el comprobante que se firma al recibir plata.

Nace como copia del egreso, del que es el espejo, pero es un formato aparte: lo
que el pago necesite imprimir y el egreso no, va acá sin tocar el otro.
...
falta para autorizar y auditar un recaudo —de quién, contra qué banco, contra qué
...
vista de quien lo firma. Se calcula acá igual que allá —el banco debita el total
del pago—, así que las dos tienen que moverse juntas.
"""
...
# Etiqueta y valor, dos veces: los datos del pago caben en cuatro columnas.
...
class FormatoDocumentoPago(FormatoBase):
    """Comprobante de pago: datos del recaudo, su asiento, el valor en letras y las firmas."""
    ...
    def _datos(self, documento, estilos):
        """
        Los datos del recaudo, en rejilla de etiqueta y valor.
    ...
    def _lineas_asiento(self, documento):
        ...
        Hay pagos de dos formas. Unos traen solo lo que se cobra y el banco es
        una pata que no está en los detalles: la genera
        `contabilizar._movimientos_banco` debitando el total del documento.
        ...
        for detalle in documento.documentos_detalles_documento_rel.all():
            ...
            lineas.append({
                # El documento que cruza esta línea: en un pago es la factura
                # que se está cobrando, y es el dato por el que se busca el
    ...
    def _asiento(self, documento, estilos):
        ...
        se recibió, y sumar la columna entera daría débitos más créditos, que es el
